...
function App() {
  ...
  const handleMediaClick = (media, type) => {
    ...
    setIsPersonModalOpen(false); // Close person modal if open
  };
  ...
  const handleInstantResultClick = (item) => {
     ...
     if (item && (item.media_type === 'movie' || item.media_type === 'tv')) {
       // Open PlayerModal for movies/TV
       handleMediaClick(item, item.media_type); 
     } else if (item && item.media_type === 'person') {
       // Set state to show PersonDetailsModal for people
  ...
  // *** NEW: Handler to Close Person Details Modal ***
  const closePersonModal = () => {
    setIsPersonModalOpen(false);
    setSelectedPerson(null);
  };

  // Function to open Person Details Modal
  const handlePersonClick = (person) => {
    setSelectedPerson(person);
    setIsPersonModalOpen(true);
    setIsPlayerModalOpen(false); // Close player modal if open
  };

  // *** This is the key function for the transition ***
  const handleKnownItemClick = (item, mediaType) => {
    console.log("Clicked Known For Item:", item, mediaType);
    setSelectedMedia({ media: item, type: mediaType }); // Set media for PlayerModal
    setIsPersonModalOpen(false);                      // Close PersonDetailsModal
    setIsPlayerModalOpen(true);                       // Open PlayerModal
  };
